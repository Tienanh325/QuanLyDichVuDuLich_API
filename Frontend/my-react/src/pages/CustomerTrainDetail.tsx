...
const CustomerTrainDetail = () => {
  ...
  const selectedFare = ticket?.bảngGia?.[0];
  const checkoutParams = new URLSearchParams({
    serviceType: 'train',
    serviceLabel: 'Vé tàu',
    maDichVu: String(ticket?.maDichVu ?? ticket?.maVe ?? ''),
    maPhanLoaiDichVu: String(selectedFare?.maGiaVe ?? ''),
    serviceName: detail.soHieuChuyenTau || ticket?.tenDichVu || 'Chuyến tàu',
    title: `${detail.diemKhoiHanh || 'Điểm đi'} → ${detail.diemDen || 'Điểm đến'}`,
    subtitle: selectedSeats.length ? `Ghế ${selectedSeats.join(', ')}` : (detail.loaiChoMacDinh || selectedFare?.tenLoaiVe || 'Vé tàu'),
