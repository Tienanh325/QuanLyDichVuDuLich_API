  ...
  return (
    <div style={pcs}>
      ...
      <Modal title={managing ? `Quản lý: ${managing.soHieuChuyenTau}` : "Quản lý vé"} open={manageOpen} onCancel={() => setManageOpen(false)} footer={null} width={1000}>
        <Tabs items={[
          { key: "prices", label: "Bảng giá", children: <Table<veApi.GiaVe> rowKey="maGiaVe" loading={childLoading} dataSource={priceRows} pagination={false} columns={[{ title: "Loại vé", dataIndex: "tenLoaiVe" }, { title: "Giá", dataIndex: "gia", render: (v: number) => `${new Intl.NumberFormat("vi-VN").format(Number(v || 0))} đ` }, { title: "Giá gốc", dataIndex: "giaGoc", render: (v: number) => v ? `${new Intl.NumberFormat("vi-VN").format(Number(v))} đ` : "--" }, { title: "Số chỗ", dataIndex: "soChoTrong" }, { title: "Thuế phí", dataIndex: "thuePhi", render: (v: number) => `${new Intl.NumberFormat("vi-VN").format(Number(v || 0))} đ` }]} /> },
